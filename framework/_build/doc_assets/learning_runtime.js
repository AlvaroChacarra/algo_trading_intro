...
function focusModalOwner(){
  if(!modalBackgroundOwner)return;
  const selector='a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),'
    +'textarea:not([disabled]),[tabindex]:not([tabindex="-1"])';
  const target=[...modalBackgroundOwner.querySelectorAll(selector)]
    .find(node=>!node.hidden&&!node.closest('[hidden],[inert]')&&node.getClientRects().length);
...
document.addEventListener('pointerdown',event=>{
  if(!modalBackgroundOwner||modalBackgroundOwner.contains(event.target))return;
  event.preventDefault();event.stopImmediatePropagation();focusModalOwner();
},true);
document.addEventListener('focusin',event=>{
  if(modalBackgroundOwner&&!modalBackgroundOwner.contains(event.target))focusModalOwner();
},true);
