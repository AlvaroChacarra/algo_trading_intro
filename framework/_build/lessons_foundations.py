...
LESSONS.append({
    ...
    "objective": "Entender cómo CPython transforma nuestra intención en operaciones ejecutables y usar las primitivas de Python para leer un order book, calcular información y decidir.",
    "frase": "Un algoritmo es siempre lo mismo: dato <span class='dot'>→</span> cálculo <span class='dot'>→</span> decisión.",
    "concepts": [
        ("De intención humana a operaciones ejecutables",
         "Python es el lenguaje y CPython su implementación estándar: compila el source a bytecode y su VM lo ejecuta. El traceback permite un ciclo corto de escribir, ejecutar, observar y corregir.",
         "bid = 99950\nask = 100000\nspread = ask - bid   # 50\nmid = (bid + ask) / 2  # 99975"),
    ...
    "build": [
        ...
        {"title": "4. Media con un bucle", "practice": "for y acumuladores",
         ...
         "pista": "Tres piezas: parte de `total = 0`, dentro del bucle acumula con `total = total + m`, y **fuera** del bucle divide: `average = total / len(mids)`. Cuidado con la indentación de la última línea.",
        ...
        {"title": "5. Una orden, y cómo leerla", "practice": "diccionarios: crear y acceder",
         ...
         "pista": "El dict se escribe con pares `'clave': valor` separados por comas. Para leer: `order['side']`. El nocional multiplica dos campos del propio dict.",
        ...
        {"title": "6. Clasifica el mercado", "practice": "if / elif / else",
         ...
         "pista": "El orden importa: comprueba primero `<= 20`, luego `elif <= 60`; el `else` recoge todo lo demás. En cada rama, una asignación a `market_state`.",
...
LESSONS.append({
    ...
    "build": [
        ...
        {"title": "3. Blinda con try/except", "practice": "manejo de errores",
         ...
         "pista": "Estructura: `try:` con `return order_book.best_bid(book)` dentro, y `except ValueError:` que hace `return None`. Captura solo `ValueError`, no cualquier cosa.",
        ...
        {"title": "4. Lanza un error claro", "practice": "raise",
         ...
         "pista": "Un `if size <= 0:` que haga `raise ValueError('size debe ser positivo')`; si pasa el filtro, `return size`. `raise` corta la función ahí mismo.",
        ...
        {"title": "5. Combina funciones del módulo", "practice": "usar varias del módulo",
         ...
         "pista": "Primero trae los nombres: `from order_book import best_bid, best_ask`. Luego, la media de ambos sobre `book`.",
...
LESSONS.append({
    ...
    "build": [
        ...
        {"title": "3. El contrato: clase abstracta", "practice": "ABC + @abstractmethod",
         ...
         "pista": "Dentro de `Strategy(ABC)`, pon `@abstractmethod` encima de `def decide(self, imbalance): ...`. El cuerpo puede ser `...` — lo que importa es el decorador.",
        ...
        {"title": "4. Polimorfismo", "practice": "mismo método, objetos distintos",
         ...
         "pista": "Una list comprehension sobre la lista de objetos: `[s.decide(0.5) for s in strategies]`. No te importa de qué clase es cada `s` — eso es el polimorfismo.",
        ...
        {"title": "5. super() en el __init__", "practice": "reutilizar la base",
         ...
         "pista": "El `__init__` de la subclase tiene una sola línea: `super().__init__('momentum')`. `super()` te da el `__init__` del padre sin tener que nombrarlo.",
    ...
    "aux": [
        ...
        {"title": "T1. Alertas de servidor (mismo patrón, otro mundo)",
         ...
         "statement": "Un sistema de monitorización tiene una `Alerta` abstracta con `revisar(valor) -> str`. "
                       "Define `AlertaCPU` (devuelve `'CRÍTICO'` si valor > 90, si no `'ok'`) y "
                       "`AlertaDisco` (`'CRÍTICO'` si valor > 80, si no `'ok'`). Recorre "
                       "`[AlertaCPU(), AlertaDisco()]` midiendo el valor 85 y guarda `estados`. "
                       "Es el mismo bucle polimórfico que la familia `Strategy`.",
         "given": "from abc import ABC, abstractmethod\n\nclass Alerta(ABC):\n    @abstractmethod\n    def revisar(self, valor): ...\n",
         "starter": "# define AlertaCPU y AlertaDisco, luego estados = [...] con valor 85\nestados = None\n",
         "validator": "assert estados == ['ok', 'CRÍTICO'], 'a 85: la CPU aún respira, el disco ya no'\nprint('ok — el polimorfismo era de programar, no de trading')",
         "pista": "Calca la familia `Strategy`: cada subclase implementa `revisar(self, valor)` con un "
                  "ternario. El bucle final es `[a.revisar(85) for a in [AlertaCPU(), AlertaDisco()]]`.",
         "solution": "from abc import ABC, abstractmethod\n\nclass Alerta(ABC):\n    @abstractmethod\n    def revisar(self, valor): ...\n\nclass AlertaCPU(Alerta):\n    def revisar(self, valor):\n        return 'CRÍTICO' if valor > 90 else 'ok'\n\nclass AlertaDisco(Alerta):\n    def revisar(self, valor):\n        return 'CRÍTICO' if valor > 80 else 'ok'\n\nestados = [a.revisar(85) for a in [AlertaCPU(), AlertaDisco()]]"},
    ],
