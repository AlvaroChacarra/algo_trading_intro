/* L4 — taller de órdenes: Order(side, price, size) en vivo */
(function(){
"use strict";
...
function update(){
  ...
  $('#ow-repr').textContent=`Order(${side} ${size} @ ${price})`;
  $('#ow-not').textContent=fmtF(notional);
  const cfEl=$('#ow-cf');
  cfEl.textContent=(cf>0?'+':'')+fmtF(cf);
  cfEl.className=cf>0?'pos':cf<0?'neg':'';
  $('#ow-log').textContent=`» order = Order('${side}', ${price}, ${size})  ·  order.notional() = ${fmtF(notional)}  ·  Fill('${side}', ${price}, ${size}).cash_flow() = ${fmtF(cf)}`;
}
