...
**Idea central:** composición (objetos que **contienen** objetos) y encapsulación (estado **interno** detrás de una API pública). Nacen las dos piezas serias del motor: el libro que se deja preguntar y el contable que nunca pierde la cuenta.
...
- Usa el inspector de invariantes. Primero aplica un Fill por la API: caja y posición cambian juntas, estado consistente. Después pulsa «tocar `_cash` a mano»: Python lo permite, pero el estado aparece inconsistente.
- Cierre de una frase: **la API pública coordina el cambio; el guion bajo comunica implementación interna, no impone seguridad**.
...
- Notebook + gimnasio (15 drills: composición, estado interno, property y el gran reto libro+contable).
