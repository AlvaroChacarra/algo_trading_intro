# Clase 15 — Práctica acumulativa pública (guía de implementación)

Cierre de práctica del curso. No es una lección con notebook: es un test que se
genera. **No usar como examen oficial:** las respuestas están divulgadas por la
naturaleza pública del repositorio.
...
- `generate_exam.py` sin `--seed` emite: `examen.html` (alumno), `examen_con_respuestas.html`
  ...
  Con `--seed N>0` emite una variante `examen_sN.html` equilibrada para práctica adicional; nunca una convocatoria oficial.
...
Al corregirse, práctica y checkpoint emiten un código copiable tipo
`AT26-L15P-S0-R34-W3-B3-N8.12-43`. Sirve para feedback, no para acreditar la
nota oficial. Se valida con:
```bash
...
## Notas
- Mantener exactamente 40 preguntas canónicas de práctica.
