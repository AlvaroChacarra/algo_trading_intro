...
| Push/PR y merge | ÁMBAR | Técnicamente listo para PR; merge condicionado a CI Chromium/WebKit/JupyterLite green sobre el SHA publicado y a checks requeridos. |
...
| Evidencia browser, dry-run y privacidad | Abierta y externa; no puede declararse desde este checkout local. |
...
| Python | 517/517 tests en el entorno local Python 3.12; la matriz CI autoritativa sigue fijada a Python 3.11.9. |
...
| Contratos Node | 44/44: Pages, Desktop y evidencia independiente. |
...
1. Publicar la rama con autorización, abrir PR y exigir green de `curso` y
   `pages` sobre el SHA exacto. Descargar y validar los artefactos Desktop,
   WebKit-site y los 14 shards JupyterLite.
2. Verificar branch protection y checks requeridos antes del merge.
...
- **GO local y GO para PR técnico.**
- **NO-GO merge** hasta CI browser green y checks requeridos en el SHA del PR.
